...
def comment_modify(request, comment_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            comment.modify_date = timezone.now()    # 수정한 날짜 입력시킴
            comment.save()  #실저장
            return redirect('blog:detail', post_id=comment.post_id)
    else:
        form = CommentForm(instance=comment)    # 댓글에 입력된 내용 그대로 가져오기 (instance)
    context = {'form': form}
